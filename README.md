...
#### Logger
A flexible logging system for debugging and error tracking.
...
// Logs a message with the specified level, source file, and line number

bool register_log_handler(log_handler_fn handler, void* user_data, log_level min_level);
// Registers a custom log handler function
...
// Clears all registered log handlers
...
void my_log_handler(const log_message* message, void* user_data) {
    FILE* log_file = (FILE*)user_data;
    fprintf(log_file, "[%s] %s:%d - %s\n", 
           message->level_str, message->file, message->line, 
           message->formatted_message);
}

void example() {
    // Enable timestamp display in logs
    print_time_in_log = true;
    
    // Set global log level
    set_log_level(DEBUG);
    
    // Use built-in logging macros
    ...
    register_log_handler(my_log_handler, log_file, INFO);
    
    // Log messages will now go to the default handler and our custom handler
    ilog("This message goes to console and file");
    
    // Error logging example (note: this would exit the program)
    // elog("This is a fatal error"); 
    
    // Clean up
    unregister_log_handler(my_log_handler);
    fclose(log_file);
}
```
